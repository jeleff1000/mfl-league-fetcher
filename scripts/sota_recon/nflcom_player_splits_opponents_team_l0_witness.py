"""Measure Opponents-by-Team/L0 defensive witnesses."""
...
OUT = Path(r"D:/league-history-data/nfl/derived/validation/sota_recon_master") / "nflcom_player_splits_opponents_team_l0_witness.json"
...
MAPPINGS = {"ast": "def_tackle_assists", "int": "def_interceptions", "pdef": "def_pass_defended", "sck": "def_sacks", "sfty": "def_safeties", "solo": "def_tackles_solo", "tds": "def_int_ret_td", "total": "def_tackles_combined", "yds": "def_interception_yards"}
...
def _identity(con):
    ...
    return {"complete_n": len(complete), "total_eq_solo_plus_ast": sum(int(a == b + c) for a, b, c in complete), "total_exceeds_parts": sum(int(a > b + c) for a, b, c in complete), "parts_exceed_total": sum(int(a < b + c) for a, b, c in complete)}


def measure():
    con = duckdb.connect()
    source = _q(SOURCE / "**/*.parquet")
    con.execute(f"""CREATE TEMP TABLE src AS SELECT DISTINCT nflcom_slug,season,split_value,_lost_column,ast,int,pdef,sck,sfty,solo,tds,total,yds,lng,avg FROM read_parquet('{source}',union_by_name=true) WHERE _table='Opponents by Team' AND _layout='player_splits_L0'""")
    ...
    candidates = {}
    for source_field, canonicals in {"solo": ("def_tackles_solo", "def_tackles_combined", "def_tackle_assists", "def_tackles_with_assist"), "total": ("def_tackles_combined", "def_tackles_solo", "def_tackle_assists", "def_tackles_with_assist"), "ast": ("def_tackle_assists", "def_tackles_solo", "def_tackles_combined", "def_tackles_with_assist")}.items():
    ...
    return {"measurement_only": True, "source_table": "Opponents by Team", "layout": "player_splits_L0", "subject_plane": "sum 48 disjoint opponent-team rows to player-season", "source_rows_after_exact_dedup": int(con.execute("SELECT COUNT(*) FROM src").fetchone()[0]), "source_players": int(con.execute("SELECT COUNT(DISTINCT nflcom_slug) FROM src").fetchone()[0]), "source_split_values": int(con.execute("SELECT COUNT(DISTINCT split_value) FROM src").fetchone()[0]), "lost_columns": [{"column": str(a) if a is not None else None, "rows": int(b)} for a, b in loss], "identity": _identity(con), "fields": fields, "candidate_matrix": candidates, "target_aggregation": "SUM weekly REG by (NFL_player_id, year)", "no_state_change": True}
