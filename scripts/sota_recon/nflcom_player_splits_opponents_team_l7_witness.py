"""Measure Opponents-by-Team/L7 kicking witnesses."""
...
OUT = Path(r"D:/league-history-data/nfl/derived/validation/sota_recon_master") / "nflcom_player_splits_opponents_team_l7_witness.json"
...
def _denominator(con, v26):
    ...
    target = con.execute(f"SELECT COUNT(*) n,SUM(CASE WHEN fg_made<=fg_att THEN 1 ELSE 0 END) ok FROM read_parquet('{v26}') WHERE season_type='REG' AND fg_made IS NOT NULL AND fg_att IS NOT NULL").fetchone()
...
def measure():
    ...
    con.execute(f"""CREATE TEMP TABLE src AS SELECT DISTINCT nflcom_slug,season,split_value,_lost_column,g,fg_att,fgm FROM read_parquet('{source}',union_by_name=true) WHERE _table='Opponents by Team' AND _layout='player_splits_L7'""")
    ...
    return {"measurement_only": True, "source_table": "Opponents by Team", "layout": "player_splits_L7", "subject_plane": "sum 48 opponent-team rows to player-season", "source_rows_after_exact_dedup": row_count, "source_players": int(con.execute("SELECT COUNT(DISTINCT nflcom_slug) FROM src").fetchone()[0]), "source_split_values": int(con.execute("SELECT COUNT(DISTINCT split_value) FROM src").fetchone()[0]), "lost_columns": [{"column": str(a) if a is not None else None, "rows": int(b)} for a, b in loss], "fields": fields, "candidate_matrix": candidates, "denominator_checks": _denominator(con, v26), "unmapped_fields": {"g": {"status": "NO_EXISTING_CANONICAL_FOUND", "schema_search": ["games_played", "games", "g"], "schema_matches": []}}, "excluded_pct": {"rows": row_count, "populated_n": 0, "status": "CAPTURE_LOST"}, "target_aggregation": "SUM weekly REG by (NFL_player_id, year)", "no_state_change": True}
