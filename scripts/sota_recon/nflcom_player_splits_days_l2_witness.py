"""Measure the recoverable Days/L2 receiving block of player splits."""
...
OUT = Path(r"D:/league-history-data/nfl/derived/validation/sota_recon_master") / "nflcom_player_splits_days_l2_witness.json"
...
def measure() -> dict:
    ...
    con.execute(f"""CREATE TEMP TABLE src AS SELECT DISTINCT nflcom_slug,season,split_value,_lost_column,"1st","20",lng,rec,td,yds FROM read_parquet('{source}',union_by_name=true) WHERE _table='Days' AND _layout='player_splits_L2'""")
    ...
    return {"measurement_only":True,"source_table":"Days","layout":"player_splits_L2","subject_plane":"weekly summed to player-season","source_rows_after_exact_dedup":int(con.execute("SELECT COUNT(*) FROM src").fetchone()[0]),"source_players":int(con.execute("SELECT COUNT(DISTINCT nflcom_slug) FROM src").fetchone()[0]),"lost_columns":[{"column":str(a) if a is not None else None,"rows":int(b)} for a,b in loss],"fields":fields,"candidate_matrix":candidate_matrix,"target_aggregation":"SUM weekly REG by (NFL_player_id, year); MAX for long","no_state_change":True}
