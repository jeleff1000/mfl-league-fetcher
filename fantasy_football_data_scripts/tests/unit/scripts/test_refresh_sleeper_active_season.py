...
def test_saved_current_identity_extends_imported_predecessor_chain(tmp_path, monkeypatch):
    """A current context ID must be validated as the successor to imported history."""
    from refresh_sleeper_active_season import _build_context
    from multi_league.data_fetchers.sleeper import sleeper_api_client
