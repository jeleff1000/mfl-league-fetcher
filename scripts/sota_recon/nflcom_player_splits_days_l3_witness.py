"""Measure the recoverable Days/L3 rushing block of player splits."""
...
OUT = Path(r"D:/league-history-data/nfl/derived/validation/sota_recon_master") / "nflcom_player_splits_days_l3_witness.json"
...
def measure() -> dict:
    ...
    con.execute(f"""CREATE TEMP TABLE src AS SELECT DISTINCT nflcom_slug,season,split_value,_lost_column,att,lng,td,yds FROM read_parquet('{source}',union_by_name=true) WHERE _table='Days' AND _layout='player_splits_L3'""")
