"""Measure Opponents-by-Team/L1 fumble witnesses."""
...
OUT = Path(r"D:/league-history-data/nfl/derived/validation/sota_recon_master") / "nflcom_player_splits_opponents_team_l1_witness.json"
...
def measure():
    ...
    con.execute(f"""CREATE TEMP TABLE src AS SELECT DISTINCT nflcom_slug,season,split_value,_lost_column,ff,fum,lost,opp_fr,own_fr FROM read_parquet('{source}',union_by_name=true) WHERE _table='Opponents by Team' AND _layout='player_splits_L1'""")
    ...
    return {"measurement_only": True, "source_table": "Opponents by Team", "layout": "player_splits_L1", "subject_plane": "sum 48 disjoint opponent-team rows to player-season", "source_rows_after_exact_dedup": int(con.execute("SELECT COUNT(*) FROM src").fetchone()[0]), "source_players": int(con.execute("SELECT COUNT(DISTINCT nflcom_slug) FROM src").fetchone()[0]), "source_split_values": int(con.execute("SELECT COUNT(DISTINCT split_value) FROM src").fetchone()[0]), "lost_columns": [{"column": str(a) if a is not None else None, "rows": int(b)} for a, b in loss], "fields": fields, "candidate_matrix": candidates, "denominator_checks": _denominator(con, v26), "target_aggregation": "SUM weekly REG by (NFL_player_id, year)", "no_state_change": True}
