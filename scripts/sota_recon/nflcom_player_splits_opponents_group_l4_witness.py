"""Measure Opponents-by-Group/L4 combined return witnesses."""
...
OUT = Path(r"D:/league-history-data/nfl/derived/validation/sota_recon_master") / "nflcom_player_splits_opponents_group_l4_witness.json"
...
def measure():
    ...
    con.execute(f"""CREATE TEMP TABLE raw_src AS SELECT * FROM read_parquet('{source}',union_by_name=true) WHERE _table='Opponents by Group' AND _layout='player_splits_L4'""")
    con.execute("""CREATE TEMP TABLE src AS SELECT DISTINCT nflcom_slug,season,split_value,_lost_column,ret,yds,td,lng,fum,fc,\"20\",\"40\" FROM raw_src WHERE split_value IN ('vs AFC Teams','vs NFC Teams')""")
    ...
    v26 = _q(latest_v26())
    candidates = {
        "ret": [_measure(con, "ret", "SUM(v.kickoff_returns)+SUM(v.punt_returns)", "SUM kickoffs + punts", "kickoff_returns + punt_returns", v26), _measure(con, "ret", "SUM(v.kickoff_returns)", "SUM kickoff only", "kickoff_returns", v26), _measure(con, "ret", "SUM(v.punt_returns)", "SUM punt only", "punt_returns", v26)],
        "yds": [_measure(con, "yds", "SUM(v.total_return_yards)", "SUM total_return_yards", "total_return_yards", v26), _measure(con, "yds", "SUM(v.kickoff_return_yards)+SUM(v.punt_return_yards)", "SUM kickoff + punt yards", "kickoff_return_yards + punt_return_yards", v26)],
        "td": [_measure(con, "td", "SUM(v.kickoff_return_tds)+SUM(v.punt_return_tds)", "SUM kickoffs + punts", "kickoff_return_tds + punt_return_tds", v26), _measure(con, "td", "SUM(v.kickoff_return_tds)", "SUM kickoff only", "kickoff_return_tds", v26), _measure(con, "td", "SUM(v.punt_return_tds)", "SUM punt only", "punt_return_tds", v26)],
        "lng": [_measure(con, "lng", "GREATEST(MAX(v.kickoff_return_long),MAX(v.punt_return_long))", "MAX kickoffs/punts", "MAX(kickoff_return_long, punt_return_long)", v26), _measure(con, "lng", "MAX(v.kickoff_return_long)", "MAX kickoff only", "kickoff_return_long", v26), _measure(con, "lng", "MAX(v.punt_return_long)", "MAX punt only", "punt_return_long", v26)],
    }
