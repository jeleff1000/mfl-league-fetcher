from scripts.sota_recon.nflcom_player_splits_opponents_team_l3_witness import measure


def test_opponents_team_l3_witness_is_measurement_only():
    result = measure()
    ...
    assert checks["satisfy_td_le_att"] <= checks["rows"]
    assert checks["rows"] - checks["satisfy_td_le_att"] == 3
