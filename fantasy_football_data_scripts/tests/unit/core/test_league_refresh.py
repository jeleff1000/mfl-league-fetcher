...
def test_future_schedule_uses_team_name_when_manager_is_disambiguated():
    from multi_league.core.league_refresh import resolve_active_schedule_franchise_ids
    ...
    class LocalDB:
        ...
        def read_table(table_name, year=None):
            if table_name == "player_fantasy":
                # The roster witness predates display-name disambiguation and
                # carries the owner's unsuffixed synthetic identity.
                return pd.DataFrame(
                    {
                        "year": [2026],
                        "manager_guid": ["--hidden--"],
...
def test_weekly_publish_selects_source_and_rebuilt_homepage_tables():
    ...
    try:
        ...
        conn.execute("CREATE TABLE public.player_fantasy_season (db_name VARCHAR, year INTEGER, NFL_player_id VARCHAR)")
        conn.execute("CREATE TABLE public.matchup_season (db_name VARCHAR, year INTEGER, franchise_id VARCHAR)")
...
def test_sleeper_missing_successor_requires_a_persisted_native_identity():
    from scripts.refresh_sleeper_active_season import _resolve_active_renewal
...
def test_sleeper_missing_successor_never_scans_unrelated_member_leagues():
    from scripts.refresh_sleeper_active_season import _resolve_active_renewal
    ...
    assert _resolve_active_renewal(
        Client(), seed_league_id="saved", active_year=2026,
