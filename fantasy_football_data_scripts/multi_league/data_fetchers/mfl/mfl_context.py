...
Mirrors FleaflickerContext. One important MFL quirk: league IDs are
per-SEASON namespaces (the same numeric ID can be a different league in a
different year), so ``league_ids`` maps year -> that season's league id as
resolved from the league's own history links.
"""
...
def resolve_years_to_fetch(ctx: MFLContext, year_filter: YearFilter = None) -> list[int]:
...
class MFLContext:
    ...
    def __post_init__(self):
        ...
        if self.data_directory is None:
            ...
            self.data_directory = Path.home() / "fantasy_football_data" / f"mfl_{safe_name}"
        else:
    ...
    def _validate(self):
        ...
        if self.start_year < 1997 or self.start_year > current + 1:
    ...
    def save(self, path: str | Path | None = None) -> Path:
        target = Path(path) if path else self.data_directory / "mfl_context.json"
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(json.dumps(self.to_dict(), indent=2, default=str), encoding="utf-8")
        return target

    @classmethod
    def from_dict(cls, payload: dict[str, Any]) -> MFLContext:
        data = dict(payload)
    ...
    @classmethod
    def load(cls, path: str | Path) -> MFLContext:
