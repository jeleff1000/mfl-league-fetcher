#!/usr/bin/env python
"""Approve safe resolved newspaper player identity candidate rows.

This station promotes candidate-only identity evidence into
`newspaper_final.player_identity_candidate`. It does not use those candidates to
rewrite scoring/PBP/stat rows; it only preserves the candidate bridge as its own
auditable final table row.
"""
...
DEFAULT_OUT_ROOT = DEFAULT_ROOT / "resolved_safe_identity_candidate_decisions"
...
AUDIT_FIELDS = [
    "safe_identity_candidate_decision_run_id",
...
RUN_FIELDS = [
    ...
    "approved_count",
    "held_count",
    "approved_match_method_counts_json",
...
def load_rows(con: duckdb.DuckDBPyConnection, decision_run_id: str) -> list[dict[str, Any]]:
    return query_dicts(
        ...
        WHERE resolved_package_decision_run_id = ?
          AND target_table = 'player_identity_candidate'
...
def row_is_safe(row: dict[str, Any]) -> tuple[bool, str, dict[str, str]]:
    if clean(row.get("route_to_lane")) != "identity_bridge_review":
        return False, "not in identity_bridge_review lane", {}
    proposed = parse_obj(row.get("proposed_fields_json"))
    fields = {
        "raw_player_name": clean(proposed.get("raw_player_name")),
        "resolved_player": clean(proposed.get("resolved_player")),
        "NFL_player_id": clean(proposed.get("NFL_player_id")),
        "player_week": clean(proposed.get("player_week")),
        "nfl_team": clean(proposed.get("nfl_team")),
        "confidence_score": clean(proposed.get("confidence_score")),
        "match_method": clean(proposed.get("match_method")),
    }
    for required in ["raw_player_name", "resolved_player", "NFL_player_id", "player_week", "nfl_team", "match_method"]:
    ...
    if not source_date_matches(row, proposed):
        return False, "source document date does not match boxscore_id", fields
    evidence = clean(row.get("evidence_text")) or clean(proposed.get("evidence_text"))
    if not evidence:
        return False, "missing evidence text", fields
    if not evidence_has_name(evidence, fields["raw_player_name"]):
        return False, f"evidence does not name raw player {fields['raw_player_name']}", fields
    if "candidate" not in clean(proposed.get("review_status")).lower():
        return False, "row is not marked candidate-only", fields
    return True, "candidate-only identity row with source-date evidence", fields
...
def build_rows(
    ...
    for row in rows:
        ...
        audit_row = {
            "safe_identity_candidate_decision_run_id": run_id,
            ...
            "raw_player_name": fields.get("raw_player_name", ""),
            "resolved_player": fields.get("resolved_player", ""),
            "NFL_player_id": fields.get("NFL_player_id", ""),
            "player_week": fields.get("player_week", ""),
            "nfl_team": fields.get("nfl_team", ""),
        ...
        if ok:
            approved.append({
                ...
                "route_to_lane": "local_final_player_identity_candidate",
                "target_table": clean(row.get("target_table")),
                "target_entity_key": clean(row.get("target_entity_key")),
                "boxscore_id": clean(row.get("boxscore_id")),
                "notes": reason,
...
def ensure_tables(con: duckdb.DuckDBPyConnection) -> None:
    ...
    con.execute(f"CREATE TABLE IF NOT EXISTS newspaper_review.resolved_safe_identity_candidate_decision_item ({item_defs})")
    con.execute(f"CREATE TABLE IF NOT EXISTS newspaper_review.resolved_safe_identity_candidate_decision_run ({run_defs})")
...
def persist(db_path: Path, run_row: dict[str, Any], audit_rows: list[dict[str, Any]]) -> None:
    ...
    try:
        ensure_tables(con)
        run_id = clean(run_row.get("safe_identity_candidate_decision_run_id"))
        con.execute(
            "DELETE FROM newspaper_review.resolved_safe_identity_candidate_decision_run WHERE safe_identity_candidate_decision_run_id = ?",
            [run_id],
        )
        con.execute(
            "DELETE FROM newspaper_review.resolved_safe_identity_candidate_decision_item WHERE safe_identity_candidate_decision_run_id = ?",
            [run_id],
        )
        insert_rows(con, "resolved_safe_identity_candidate_decision_run", [run_row], RUN_FIELDS)
        insert_rows(con, "resolved_safe_identity_candidate_decision_item", audit_rows, AUDIT_FIELDS)
    finally:
        con.close()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--label", default="resolved_safe_identity_candidate_decisions")
...
def main() -> int:
    ...
    try:
        ...
        source_rows = load_rows(con, decision_run_id)
    finally:
        con.close()
    ...
    held_csv = out_dir / "held_identity_candidate_rows.csv"
    audit_csv = out_dir / "safe_identity_candidate_decision_audit.csv"
    ...
    approved_methods = Counter(row["match_method"] for row in audit if row["decision_status"] == "approved")
    ...
    summary = {
        ...
        "held_count": len(held),
        "approved_match_method_counts": dict(sorted(approved_methods.items())),
    ...
    run_row = {
        ...
        "approved_match_method_counts_json": json.dumps(summary["approved_match_method_counts"], sort_keys=True),
