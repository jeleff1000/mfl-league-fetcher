"""Measure Opponents-by-Group/L3 rushing witnesses."""
...
OUT = Path(r"D:/league-history-data/nfl/derived/validation/sota_recon_master") / "nflcom_player_splits_opponents_group_l3_witness.json"
...
def measure():
    ...
    con.execute(f"""CREATE TEMP TABLE raw_src AS SELECT * FROM read_parquet('{source}',union_by_name=true) WHERE _table='Opponents by Group' AND _layout='player_splits_L3'""")
    con.execute("""CREATE TEMP TABLE src AS SELECT DISTINCT nflcom_slug,season,split_value,_lost_column,att,lng,td,yds FROM raw_src WHERE split_value IN ('vs AFC Teams','vs NFC Teams')""")
