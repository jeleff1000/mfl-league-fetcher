#!/usr/bin/env python
"""Prepare quality-review handoffs from non-ready newspaper packages.

Quality review is for materialized rows that have useful atoms but are not ready
for direct promotion. This station packages them by target table with evidence,
proposed fields, and a recommended next action so the conveyor can keep moving:
promote after review, send to semantic/OCR follow-up, hold as context, or reject.
"""
...
DEFAULT_OUT_ROOT = DEFAULT_ROOT / "quality_review_preps"

PACKAGE_FIELDS = [
    "quality_review_prep_run_id",
    "action_queue_run_id",
    "package_run_id",
...
def load_quality_actions(con: duckdb.DuckDBPyConnection, action_queue_run_id: str, limit: int) -> list[dict[str, Any]]:
    sql = """
        SELECT *
        FROM newspaper_review.llm_conveyor_action_queue
        WHERE action_queue_run_id = ?
          AND action_type = 'quality_review'
        ORDER BY priority, target_table, boxscore_id, max_confidence_score DESC, target_entity_key
    """
...
def recommended_next_action(row: dict[str, Any]) -> str:
    ...
    max_score = parse_float(row.get("max_confidence_score"))
    proposed = parse_json_obj(row.get("proposed_fields_json"))
    status_text = " ".join(
        clean(proposed.get(key)).lower()
        for key in ["reconciliation_status", "review_status", "promotion_status"]
        if clean(proposed.get(key))
    )

    if "preview" in status_text:
        return "hold_as_context_until_final_score_or_stats_found"
    if "cross_game" in status_text:
        return "send_to_semantic_route_review"
    if "team_label" in status_text or "needs_resolution" in status_text:
        return "resolve_team_label_before_promotion"
    if "visual" in status_text or "scoreless_tie" in status_text:
        return "visual_verify_then_promote_or_hold"
    if target_table in {"lineup_participation", "player_identity_candidate"}:
        return "resolve_player_or_team_identity_then_promote"
    if target_table in {"scoring_event", "play_by_play_event"} and confidence_bar == "medium":
        return "second_pass_article_review_for_event_detail"
    if confidence_bar == "high" and max_score >= 75:
        return "evidence_check_then_promote"
    return "quality_review_decide_promote_followup_or_reject"
...
def build_rows(
    ...
    for action in actions:
        ...
        merged = {**package, **action}
        next_action = recommended_next_action(merged)
        package_rows.append({
            "quality_review_prep_run_id": prep_run_id,
...
def create_tables(con: duckdb.DuckDBPyConnection) -> None:
    ...
    for table_name, fields in [
        ("llm_quality_review_prep_package", PACKAGE_FIELDS),
        ("llm_quality_review_prep_item", ITEM_FIELDS),
    ]:
    ...
    con.execute(
        """
        CREATE TABLE IF NOT EXISTS newspaper_review.llm_quality_review_prep_run (
          quality_review_prep_run_id VARCHAR,
...
def persist(
    ...
    try:
        ...
        for table in [
            ...
            con.execute(
                f"DELETE FROM newspaper_review.{table} WHERE quality_review_prep_run_id = ?",
                [prep_run_id],
            )
        insert_rows(con, "llm_quality_review_prep_package", package_rows, PACKAGE_FIELDS)
        insert_rows(con, "llm_quality_review_prep_item", item_rows, ITEM_FIELDS)
        insert_rows(con, "llm_quality_review_prep_run", [{
            "quality_review_prep_run_id": prep_run_id,
            "action_queue_run_id": action_queue_run_id,
            "package_run_id": package_run_id,
...
def write_markdown(path: Path, summary: dict[str, Any], package_rows: list[dict[str, Any]], item_rows: list[dict[str, Any]]) -> None:
    ...
    lines = [
        "# Newspaper Quality Review Prep",
        "",
        f"Created: `{summary['created_at_utc']}`",
        f"Prep run: `{summary['quality_review_prep_run_id']}`",
        f"Action queue run: `{summary['action_queue_run_id']}`",
        f"Package run: `{summary['package_run_id']}`",
        "",
        "## Counts",
        "",
        f"- Quality-review packages: `{summary['package_count']}`",
    ...
    for package in package_rows:
        ...
        lines.extend([
            ...
            f"- Recommended next action: `{clean(package.get('recommended_next_action'))}`",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--label", default="newspaper_quality_review_prep")
...
def main() -> int:
    ...
    try:
        ...
        actions = load_quality_actions(con, action_queue_run_id, args.limit)
    ...
    summary = {
        ...
        "recommended_next_action_counts": dict(Counter(row["recommended_next_action"] for row in package_rows)),
        "boxscore_count": len({row["boxscore_id"] for row in package_rows if row["boxscore_id"]}),
    }
    summary_path = out_dir / "summary.json"
    write_csv(out_dir / "quality_review_package_manifest.csv", package_rows, PACKAGE_FIELDS)
    write_csv(out_dir / "quality_review_item_manifest.csv", item_rows, ITEM_FIELDS)
    write_json(summary_path, summary)
    write_markdown(out_dir / "quality_review_work_order.md", summary, package_rows, item_rows)
